...
;;
;; ================================================================
;; Wed Mar 09 21:46:48 HKT 2011 - carltonf
;; 1. adding `pronouncing' support
;; 
;;; Code:
...
(defvar sdcv-mode-font-lock-keywords
  '(
    ...
    ;;;; wordnet 1.7 support
    ;; property
    ("^ *\\(n\\|adj\\|v\\|adv\\) [0-9]*:" . (1 font-lock-function-name-face))
    ;; syn and ant
    ("\\(\\[syn:[^\]0-9]*\\]\\)" . (1 font-lock-variable-name-face))
    ("\\(\\[ant:[^\]0-9]*\\]\\)" . (1 font-lock-preprocessor-face))
    ;; word name
...
(defvar sdcv-mode-map
  (let ((map (make-sparse-keymap)))
    ...
    (define-key map "c" 'sdcv-clone-word-buffer)
    (define-key map "w" 'sdcv-pronounce-current-word)
    (define-key map "W" 'sdcv-pronounce-word)
    map)
...
(defvar sdcv-current-searched-word "word"
  "The current searched word")

(defun sdcv-do-lookup (word)
  "Send the word to the sdcv process and return the result."
  (setq sdcv-current-searched-word word)
...
; ================================================================
(defun sdcv-pronounce-current-word ()
  (interactive)
  (start-process "pronounce_word.sh" nil sdcv-pronounce-word-path
		 sdcv-current-searched-word))
(defun sdcv-pronounce-word ()
  (interactive)
  (start-process "pronounce_word.sh" nil sdcv-pronounce-word-path
		 (sdcv-current-word)))

(defun sdcv-clone-word-buffer ()
  (interactive)
  (save-excursion
    (save-match-data
      (goto-char (point-min))
      (re-search-forward "Found [0-9]+ items?, similar to \\([^.]+\\).")
      (clone-buffer (format "sdcv-<%s>" (match-string 1)) t))))
...
(defvar sdcv-pronounce-word-path "/home/icesnow/bin/pronounce_word.sh"
  "Word pronouncing scripts.")
